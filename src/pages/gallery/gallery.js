...
import './gallery.scss';
import { Gallery as GalleryData } from '../../utils/data';

function Gallery() {
  const [catsHidden, setCatsHidden] = useState(true);

  return (
    <div className="gallery">
      <div className="hidden-cat" onClick={() => setCatsHidden(!catsHidden)}>
        <svg
          width="75px"
          height="75px"
          viewBox="0 0 512 512"
      ...
      <div className="gallery-container">
        {GalleryData.map((item, index) => (
          <>
            {item.free ? (
              <div key={index} className={`gallery-img double-null`} />
            ) : (
              <div key={index} className={`gallery-img double-${item.double} `}>
                <img
                  src={item.img}
                  alt={item.title}
                  loading="lazy"
                  className={`${item.cat ? (catsHidden ? 'hidden-cats' : 'not-hidden-cats') : ''}`}
                />
              </div>
            )}
          </>
